...
@connect(({ book_info: { list, bookInfo } }) => ({ list, bookInfo }))
//@ts-ignore
@Form.create()
export default class EditTemplate extends PureComponent<any, any> {
  ...
  public FromRender = () => {
    ...
    const { imgList, videoList } = this.state;
    const { form: { getFieldDecorator }, bookInfo, dispatch, } = this.props;
  ...
  public render() {
    ...
    return (
      <>
        ...
        <Modal
          className={'___warp'}
          title={Object.keys(bookInfo).length?'Add new book':'Edit book'}
          visible={true}
          onOk={handleOk}
