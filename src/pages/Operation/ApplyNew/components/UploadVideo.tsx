...
import CalcVideo from './CalcVideo'
...
function UploadImage(props) {
 ...
 const onChange = (evt) => {
  ...
  if(!isAssetTypeAnVideo(file.type)){
    props.onUpload({msg:'格式不正确'});
    return
  }
  new CalcVideo(file,1)
  .then((res)=>{
 ...
 return (
  ...
  <Row type="flex">
      <Col style={{width:'150px'}}>
         ...
         <Button icon="cloud-upload" onClick={onUploadChange}>上传视频</Button>
      </Col>
      <Col span={12}>
      <a href={defaulturl?defaulturl:url} target="_bank">
        {(defaulturl||url)&&<video src={defaulturl?defaulturl:url} style={{width:'100px',height:'100px'}}/>}
      </a>
      </Col>
   </Row>
   </>
