...
import { create, fetch, patch, fetchCates, fetchStatuses, patchStatus, batchModify, deleteTool, fetchClients, getRoomGifts } from '../services/gift';
function modelInit() {
  return {
    state: {
      list: [],
    ...
    reducers: {
      ...
      saveRoomGifts(state, { payload: { roomGifts } }) {
        return { ...state, roomGifts };
      },
    ...
    effects: {
      ...
      *getRoomGifts(payload, { call, put }) {
        const { data } = yield call(getRoomGifts);
        // console.info('fetchPositions', data);
        yield put({
          type: 'saveRoomGifts',
          payload: {
    ...
    subscriptions: {
      setup({ dispatch, history }) {
        return history.listen(({ pathname, search }) => {
          ...
          if (pathname === '/operation/gift/list') {
            dispatch({ type: 'fetch', payload: query });
            dispatch({ type: 'fetchClients' });
            dispatch({ type: 'getRoomGifts' });
