...
import { create, fetch, patch, fetchCates, fetchStatuses, patchStatus, batchModify, deleteTool, fetchClients } from '../services/show';
function modelInit() {
  return {
    state: {
      list: [],
    ...
    effects: {
      ...
      * fetch({ payload: { ...values } }, { call, put }) {
        ...
        const { data } = yield call(fetch, values);
        if (values.export) {
          return
        }
        const { list, total_count, total_page } = data;
    ...
    subscriptions: {
      setup({ dispatch, history }) {
        return history.listen(({ pathname, search }) => {
          ...
          if (pathname === '/operation/host/manage') {
