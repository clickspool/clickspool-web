...
@connect(({ categories: { list, total, page, pageSize } }) => ({ list, total, page, pageSize }))
class SceneCatefory extends PureComponent {
  ...
  onShowSizeChange = (current, size) => {
    this.props.dispatch({
      type: 'categories/changePageSize',
      payload: {
        page: current,
        pageSize: size
      }
    ...
    return (
      <Fragment>
        <Breadcrumb className='breadcrumb-box' separator='/'>
          ...
          <Breadcrumb.Item>{formatMessage({ id: 'menu.operation.video.category' })}</Breadcrumb.Item>
        </Breadcrumb>
        ...
        <CategoryEdit visible={this.state.adding} close={this.hideAdd.bind(this)} />
        ...
        <CategoryEdit caption={formatMessage({ id: 'app.scene.category.edit' })} id={this.state.editId} visible={!!this.state.editId} close={this.hideEdit.bind(this)} />
      </Fragment>
    );
  }
}
