...
@connect(({ memeCategories: { list } }) => ({ list }))
@Form.create()
class SceneCategoryEdit extends PureComponent {
  ...
  handleOk = async () => {
    ...
    validateFields(async errors => {
      ...
      const result = id ? await dispatch({ payload: params, type: 'memeCategories/patch' }) : await dispatch({ payload: params, type: 'memeCategories/create' });
      if (result) {
        close();
        resetFields();
      }
