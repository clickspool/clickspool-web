...
@connect(({ memeTools: { list, statuses, clients, total, page, pageSize }, memeCategories: { list: cates }, memeSubCategories: { list: positions } }) => ({ list, cates, positions, clients, statuses, total, page, pageSize }))
class List extends PureComponent {
  ...
  render() {
    ...
    let columns = [
      ...
      {
        title: formatMessage({ id: 'app.meme.tools.position' }),
    ...
    return (
      <Fragment>
        <Breadcrumb className='breadcrumb-box' separator='/'>
          ...
          <Breadcrumb.Item>{formatMessage({ id: 'menu.operation.meme.tools' })}</Breadcrumb.Item>
        </Breadcrumb>
        ...
        {this.state.editId && <ToolEdit caption={formatMessage({ id: 'app.scene.tools.edit' })} id={this.state.editId} visible={!!this.state.editId} list={list} positions={positions} clients={clients} cates={cates} close={this.hideEdit.bind(this)} />}
