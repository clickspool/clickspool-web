...
@connect(({ users_info: { loading, genders, user_interests, nation, videoMap,
  messageMap,
  voiceMap,
  tagsMap }, mcn: { list: mcnList } }) => ({
    loading, genders, user_interests, nation, videoMap,
    messageMap,
    voiceMap,
    tagsMap,
    mcnList
...
export default class AccountEdit extends React.Component {
  ...
  state = {
    fields: [
      ...
      },
      {
        ...
        properties: {
          placeholder: formatMessage({ id: 'app.user.mcn.placeholer' }),
        },
        type: 'select',
        optionList: 'mcnList',
        label: formatMessage({ id: 'app.user.mcn' }),
      },
      {
        name: 'text_price',
        options: {
  ...
  handleOk = () => {
    ...
    validateFields(async errors => {
      ...
      // else{
      //   return message.error('请上传背景')
      // }

      if (record) {
        params.append('user_id', record.id);
      }
      params.append('login_type', record.login_type);
      console.info('params__', { values, params, record });
      const result = await dispatch({ payload: { params }, type: 'users_info/updateUser' });
      if (result) {
        this.handleCancel();
      }
    });
    ...
    return memoize((field, init = {}, fieldValue, fieldError, status) => {
      ...
      if (init && field.name in init && field.name != 'user_interests' && field.name != 'user_tags') {
        if (field.type === 'upload') {
          ...
          field.options.initialValue = init[field.name] == '' ? null : moment(init[field.name], 'DD/MM/YYYY');
      ...
      if (type === 'select') {
        ...
        let optionsEl = [...optionList].map(item => {
