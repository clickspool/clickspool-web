...
import { create, fetch, patch, patchStatus, fetchSdks, fetchClients, batchModify, deleteTool, fetchVersionRange } from '../services/movies';
export default {
  state: {
    list: [],
    total: null,
  ...
  reducers: {
    ...
    saveSdks(state, { payload: { sdks } }) {
      const sdkMap = new Map([[null, '']]);
      Object.keys(sdks).map(key => {
        sdkMap.set(key, sdks[key]);
      });
      return { ...state, sdks: sdkMap };
    },
  ...
  effects: {
    ...
    *fetchSdks(payload, { call, put }) {
      const { data } = yield call(fetchSdks);
      // console.info('fetchPositions', data);
      yield put({
        type: 'saveSdks',
        payload: {
    ...
    *fetchVersionRange(payload, { call, put }) {
      const { data } = yield call(fetchVersionRange);
      // console.info('fetchPositions', data);
      yield put({
        type: 'updateState',
        payload: {
    ...
    * batchModify({ payload: { ids, status } }, { call, put }) {
      const { data } = yield call(batchModify, { ids, status });
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      return history.listen(({ pathname, search }) => {
        ...
        if (pathname === '/operation/movie/templates') {
          dispatch({ type: 'fetch', payload: query });
          dispatch({ type: 'fetchClients' });
          dispatch({ type: 'fetchSdks' });
          dispatch({ type: 'fetchVersionRange' });
        }
      });
    },
  },
