...
import { create, fetch, patch, fetchCates, fetchStatuses, patchStatus, batchModify, getImportTemplate, upload, deleteData } from '../services/advertisement';
function modelInit() {
  return {
    state: {
      list: [],
    ...
    reducers: {
      ...
      deleteUpdate(state, { payload: { deletedId } }) {
        const deleteItems = `${deletedId}`.split(',');
        let { list } = state;
        list = list.filter(item => {
          return deleteItems.indexOf(`${item.id}`) === -1;
        });
        return { ...state, list }
      },
    ...
    effects: {
      ...
      * upload({ payload: values }, { call, put }) {
        const result = yield call(upload, values);
        if (result && result.code === 0) {
          yield put({ type: 'reload' })
          return result;
        }
      ...
      * getImportTemplate(payload, { call, put }) {
        const { data } = yield call(getImportTemplate);
        if (data && data.advert_import_template) {
          window.open(data.advert_import_template);
        }
      },
      ...
      * delete({ payload: { ids } }, { call, put }) {
        console.info('id', ids);
        yield call(deleteData, { ids });
        yield put({
          type: 'deleteUpdate',
          payload: { deletedId: ids }
        });
      },
    ...
    subscriptions: {
      setup({ dispatch, history }) {
        return history.listen(({ pathname, search }) => {
          ...
          if (pathname === '/operation/advertisement') {
