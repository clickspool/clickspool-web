  ...
  effects: {
    ...
    *reload(action, { put, select }) {
      const { page, pageSize: page_size } = yield select(state => {
        return {
          page: state.faceCategories.page,
          pageSize: state.faceCategories.pageSize
        }
      });
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      return history.listen(({ pathname, search }) => {
        ...
        if (pathname === '/operation/face/category' || pathname === '/operation/face/tools' || pathname === '/operation/face/subcate') {
