...
@connect(({ faceCategories: { list } }) => ({ list }))
@Form.create()
class SceneCategoryEdit extends PureComponent {
  ...
  handleOk = async () => {
    ...
    validateFields(async errors => {
      ...
      const result = id ? await dispatch({ payload: params, type: 'faceCategories/patch' }) : await dispatch({ payload: params, type: 'faceCategories/create' });
      if (result) {
        close();
        resetFields();
      }
